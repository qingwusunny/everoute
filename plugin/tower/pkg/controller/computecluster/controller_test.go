...
var _ = Describe("elf controller", func() {
	AfterEach(func() {
		...
		err := erClient.CoreV1().ConfigMaps(towerSpace).Delete(ctx, msconst.ELFConfigmapName, metav1.DeleteOptions{})
		Expect(err).Should(BeNil())
		Eventually(func(g Gomega) {
			res, _ := erClient.CoreV1().ConfigMaps(towerSpace).Get(ctx, msconst.ELFConfigmapName, metav1.GetOptions{})
	...
	Context("add elf configmap", func() {
		It("er cluster associate elf", func() {
			erCluster := &schema.EverouteCluster{
				...
				AgentELFClusters: []schema.ObjectReference{
					{ID: "elf1"},
					{ID: "elf2"},
				},
			}
			...
			Eventually(func(g Gomega) {
				res, err := erClient.CoreV1().ConfigMaps(towerSpace).Get(ctx, msconst.ELFConfigmapName, metav1.GetOptions{})
